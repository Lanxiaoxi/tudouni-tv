...
document.addEventListener('DOMContentLoaded', function () {
    // ---- 数据源校准（2026-08-06）----
    // 背景：历史遗留的 selectedAPIs 可能含已不存在的源 key（如旧默认值 tyyszy/dyttzy/bfzy/ruyi
    ...
    if (_stored.length === 0) {
        _stored = Object.keys(API_SITES).filter(k => !API_SITES[k].adult).slice(0, 4);
    }
    selectedAPIs = _stored;
    localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));

    // 初始化API复选框
    initAPICheckboxes();

    // 自定义API功能已禁用（2026-08-06）：不再渲染自定义API列表
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源：动态取前 4 个普通源（不写死，避免源列表更新后 key 失效）
        selectedAPIs = Object.keys(API_SITES).filter(k => !API_SITES[k].adult).slice(0, 4);
        localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));
...
// 渲染自定义API列表（已注释：自定义API功能已禁用，2026-08-06）
/*
...
// 编辑/更新/取消编辑/恢复按钮 均属于自定义API功能，已禁用（2026-08-06）
/*
// 编辑自定义API
...
function updateSelectedAPIs() {
    ...
    // 自定义API已禁用（2026-08-06）：不再合并自定义源
    // const customApiCheckboxes = document.querySelectorAll('#customApisList input:checked');
    // const customApiIndices = Array.from(customApiCheckboxes).map(input => 'custom_' + input.dataset.customIndex);

    // 合并内置和自定义API
    selectedAPIs = [...builtInApis];
...
// 添加/取消添加/移除 均属于自定义API功能，已禁用（2026-08-06）
/*
// 显示添加自定义API表单
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `TudouniTV - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `TudouniTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - TudouniTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'TudouniTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'TudouniTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'TudouniTV-Settings_' + times + '.json');
}
